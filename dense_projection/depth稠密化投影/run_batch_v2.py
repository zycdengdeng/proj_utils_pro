...
# -*- coding: utf-8 -*-
"""
Dense Projection - Depth稠密化投影批量处理
输出深度图：近白远黑+稠密化，支持动静结合的点云投影
"""
...
def process_single_clip(clip_name, config, num_processes, threads_per_frame, project_root):
    ...
    if not scene_paths or not common_utils.validate_clip_paths(scene_paths, direction):
        print(f"  路径验证失败，跳过")
        return
    ...
    output_paths = common_utils.get_unified_output_paths(str(output_root), clip_name, 'depth_dense')
...
def main():
    print("\n" + "="*80)
    print("  Dense Projection - Depth稠密化投影 (深度图+稠密化)")
    print("="*80)
